...
impl Command {
    ...
    async fn render(&self, post: &mut post::Command) -> graph::Result<()> {
        for cmd in &self.render_entries {
            match cmd {
                Entry::Pipe(pipe) => post.render.push(post::render::Render::Pipe(pipe.clone())),
                Entry::Bind(index, bind) => {
                    let bind = bind.base().await?;
                    post.render
                        .push(post::render::Render::Bind(*index, bind.clone()))
                }
                Entry::Vertex(slot, buffer) => {
                    let buffer = buffer.base().await?;
                    post.render
                        .push(post::render::Render::Vertex(*slot, buffer.clone()))
                }
                Entry::Index(buffer) => {
                    let buffer = buffer.base().await?;
                    post.render.push(post::render::Render::Index(buffer.clone()))
                    // pass.set_index_buffer(buffer.slice(..), IndexFormat::Uint16);
                }
                Entry::Draw(vertices, instances) => {
                    post.render.push(post::render::Render::Draw(
                        vertices.clone(),
                        instances.clone(),
                    ))
                ...
                Entry::DrawIndexed(indices, base_vertex, instances) => {
                    post.render.push(post::render::Render::DrawIndexed(
                        indices.clone(),
                        *base_vertex,
                        instances.clone(),
                    ))
