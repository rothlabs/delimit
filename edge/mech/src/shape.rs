                ...
                Control::Table(table) => {
                    match table {
                        ...
                        Table::Hedge(control) => {
                            ...
                            let bind_layout = self
                                .gpu
                                .bind_layout(&[setup_entry, index_entry, basis_entry, control_entry, plot_entry])
                                .make()?;
                            let stride = 1;
                            let pipe_layout = self.gpu.pipe_layout(&[&bind_layout]).make()?;
                            let shader = self.gpu.shader(include_wgsl!("plot/control_matrix.wgsl"));
                            let pipe = shader.compute("main").layout(&pipe_layout).make()?;
                            let setup = self.gpu.buffer_uniform().field(order).field(count.clone()).field(stride).field(self.dimension).hub()?;
                            let plot = self.gpu.sizer(basis.clone()).mul(self.dimension).mul(stride).div(order).hub()?;
                            let bind = self
                                .gpu
                                .binder()
                                .layout(bind_layout)
                                ...
                                .entry(4, plot)
                                .hub()?;
                            compute = compute.pipe(pipe).bind(bind).dispatch(count.clone());
                        }
                    }
                }
            }
