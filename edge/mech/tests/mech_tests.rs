...
async fn nurbs() -> dom::Result<()> {
    ...
    let count = 5;
    //                        6 knots                      3 weights
    let span = gpu.hedge(vec![0.0_f32, 0., 0., 1., 1., 1., 1., 1., 1.]).await?;
    let index = gpu.hedge(vec![0_u32, 1, 2]).await?;
    let control = gpu.hedge(vec![-1.5_f32, -2.5, 0.5, 2.4, 1.4, 0.8]).await?;
    let shape = mech
        .shape(Rule::Nurbs(3))
        .span(span)
        .index(index)
